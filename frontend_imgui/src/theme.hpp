...
ImU32 whitespace_dot_color();

// Applied on top of the built-in defaults, keyed by the same names as the
// zedit.set_color() Lua API: "text", "whitespace", each TokenKind's
// lowercase name (comment/string/number/keyword/type/function/variable/
// constant), and each DiagnosticSeverity's lowercase name (error/warning/
// information/hint). Unrecognized names are stored but never consulted by
// anything, i.e. silently inert -- see the config's lack of name
// validation for why.
