...
// range.end). Updates the unnamed register and (for Delete/Change) the
// buffer, but never touches mode -- callers are responsible for entering
// Insert mode after a Change, and for re-clamping the cursor after a
// Delete (Change intentionally leaves the cursor exactly at the deletion
// point, which may be one past the last remaining character).
...
// Pastes the unnamed register before (P) or after (p) the cursor, with its
// content repeated `count` times as a single block.
